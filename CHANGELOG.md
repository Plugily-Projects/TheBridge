...
* Added arena state placeholders for PlaceholderAPI:
   * %thebridge_{arena id}:{data}% where data types are:
...
* Added permissions for increased chance of being murderer or detective, node: `thebridge.role.murderer.<1/100>` or
`thebridge.role.detective.<1/100>` the number is the amount of bonus points to give on arena join
...
to load them before thebridge starts and avoid plugin start failure
...
* /thebridge and /thebridgeadmin commands will now work in game for non ops
...
* In case of server crash players' inventories will be loaded on join from backup from thebridge minigame if exists
...
* Added thebridge.command.override permission
