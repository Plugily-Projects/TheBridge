...
public class LanguageManager {
  ...
  private static void loadProperties() {
    ...
    if (service == null) {
      Debugger.sendConsoleMsg("&c[thebridge] Locales cannot be downloaded because API website is unreachable, locales will be disabled.");
    ...
    if (service.isValidVersion()) {
      ...
      if (status == LocaleService.DownloadStatus.FAIL) {
        pluginLocale = LocaleRegistry.getByName("English");
        Debugger.sendConsoleMsg("&c[thebridge] Locale service couldn't download latest locale for plugin! English locale will be used instead!");
        return;
      } else if (status == LocaleService.DownloadStatus.SUCCESS) {
        Debugger.sendConsoleMsg("&c[thebridge] Downloaded locale " + pluginLocale.getPrefix() + " properly!");
      } else if (status == LocaleService.DownloadStatus.LATEST) {
        Debugger.sendConsoleMsg("&c[thebridge] Locale " + pluginLocale.getPrefix() + " is latest! Awesome!");
      }
    } else {
      pluginLocale = LocaleRegistry.getByName("English");
      Debugger.sendConsoleMsg("&c[thebridge] Your plugin version is too old to use latest locale! Please update plugin to access latest updates of locale!");
      return;
    }
  ...
  private static void setupLocale() {
    ...
    if (pluginLocale == null) {
      Debugger.sendConsoleMsg("&c[thebridge] Plugin locale is invalid! Using default one...");
      pluginLocale = LocaleRegistry.getByName("English");
    }
    Debugger.sendConsoleMsg("&a[thebridge] Loaded locale " + pluginLocale.getName() + " (" + pluginLocale.getOriginalName() + " ID: "
